    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative">
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-semibold text-dark-100 tracking-tight">Welcome Back</h1>
          <p className="text-sm text-dark-500 mt-1.5">Sign in to continue building habits</p>
        </div>

        {/* Login Card */}
        <motion.div
          className="bg-dark-900 border border-white/[0.06] rounded-2xl p-6 shadow-card"
          animate={
            isSubmitting
              ? { y: -4, boxShadow: '0 0 40px rgba(99, 102, 241, 0.15)' }
              : { y: 0, boxShadow: '0 1px 3px rgba(0, 0, 0, 0.3)' }
          }
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
              ...
              >
                <div className="absolute left-3 top-1/2 -translate-y-1/2 z-10">
                  <motion.div
                    animate={{ rotate: emailActive ? 15 : 0, scale: emailActive ? 1.2 : 1 }}
                    transition={fieldSpring}
                  >
                    <Mail
                      size={16}
                      className={`transition-colors duration-200 ${emailActive ? 'text-primary-400' : 'text-dark-600'}`}
                    />
                  </motion.div>
                </div>
                <input
                  type="email"
                  className={`input pl-10 ${errors.email ? 'input-error' : ''} ${emailActive ? 'ring-2 ring-primary-500/20 border-primary-500/50' : ''}`}
            ...
            <div>
              ...
              >
                <div className="absolute left-3 top-1/2 -translate-y-1/2 z-10">
                  <motion.div
                    animate={{ rotate: passwordActive ? -15 : 0, scale: passwordActive ? 1.2 : 1 }}
                    transition={fieldSpring}
                  >
                    <Lock
                      size={16}
                      className={`transition-colors duration-200 ${passwordActive ? 'text-primary-400' : 'text-dark-600'}`}
                    />
                  </motion.div>
                </div>
                <input
                  type="password"
                  className={`input pl-10 ${errors.password ? 'input-error' : ''} ${passwordActive ? 'ring-2 ring-primary-500/20 border-primary-500/50' : ''}`}
        ...
        <div className="mt-6">
          <div className="relative flex items-center justify-center mb-5">
            ...
            <span className="relative bg-dark-950 px-4 text-xs text-dark-600">or</span>
          </div>
            ...
            <AnimatePresence mode="wait">
              {!isDemoMode ? (
                ...
                >
                  ...
                  <div className="relative bg-dark-950 rounded-[11px] px-5 py-3.5 flex items-center gap-3">
                    <Sparkles size={18} className="text-primary-400 shrink-0" />
                    <div className="text-left">
                      <p className="font-medium text-dark-200 text-sm">Try Demo Account</p>
                ...
                >
                  ...
                  <p className="text-dark-500 text-sm">Watch the magic happen...</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
